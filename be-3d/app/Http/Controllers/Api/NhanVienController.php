    ...
    public function listStaff()
    {
        ...
        if (Schema::hasTable('nhat_ky_hoat_dong')) {
            ...
            $logs = $dbLogs->map(function ($l) {
                // Định dạng thời gian tương đối hoặc tuyệt đối
                $timeStr = $l->tao_luc ? $l->tao_luc->diffForHumans() : 'Vừa xong';
                // Thay thế diffForHumans tiếng Anh sang tiếng Việt cơ bản cho đồng bộ
                $timeStr = str_replace(
                    ['ago', 'minutes', 'minute', 'hours', 'hour', 'days', 'day', 'weeks', 'week', 'months', 'month', 'years', 'year', 'just now', 'before'],
                    ['trước', 'phút', 'phút', 'giờ', 'giờ', 'ngày', 'ngày', 'tuần', 'tuần', 'tháng', 'tháng', 'năm', 'năm', 'Vừa xong', 'trước'],
                    $timeStr
                );
                return [
                    'id' => $l->id,
