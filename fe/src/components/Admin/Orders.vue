<template>
  <div class="page-wrap">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="o in filteredOrders" :key="o.id">
            ...
            <td><span class="payment-badge whitespace-nowrap">{{ o.payment }}</span></td>
            <td>{{ o.date }}</td>
    ...
    <div class="modal-overlay" v-if="showDetail" @click.self="showDetail = false">
      <div class="modal-box modal-lg" v-if="selectedOrder">
          ...
          <div class="order-detail-grid">
            <div class="detail-section">
              ...
              <div class="detail-rows">
                ...
                <div class="detail-row" style="display: flex; justify-content: flex-start; gap: 4px;">
                  <span>Thanh toán:</span><strong>{{ selectedOrder.payment }} ({{ selectedOrder.payment_status
                    }})</strong>
                </div>
              </div>
            </div>
            ...
            <div class="detail-section">
              ...
              <div v-if="selectedOrder.status === 'da_huy'" class="canceled-order-banner"
                style="background: #fef2f2; border: 1px solid #fecaca; color: #dc2626; padding: 12px; border-radius: 8px; font-weight: 600; display: flex; align-items: center; gap: 8px; justify-content: center; margin-top: 10px;">
              ...
              <div class="detail-row" style="margin-top: 27px; display: flex; justify-content: flex-start; gap: 4px;">
                <span>Tổng tiền:</span><strong style="color: #dc2626; font-size: 15px;">{{ selectedOrder.total
                  }}</strong>
              </div>
              <div class="detail-row" style="display: flex; justify-content: flex-start; gap: 4px;">
        ...
        <div class="modal-footer">
          ...
          <button class="btn-primary" style="background-color:#dc2626; border-color: #3b82f6;"
            v-if="selectedOrder.status === 'cho_xu_ly'" @click="updateOrderStatus('dang_chuan_bi')">Xác nhận
            đơn</button>
          <button class="btn-primary" style="background-color: #f59e0b; border-color: #f59e0b;"
            v-if="selectedOrder.status === 'dang_chuan_bi'" @click="updateOrderStatus('dang_giao')">Giao hàng</button>
          <button class="btn-primary" style="background-color: #10b981; border-color: #10b981;"
            v-if="selectedOrder.status === 'dang_giao'" @click="updateOrderStatus('da_giao')">Hoàn tất</button>
          <button class="btn-outline" style="color: #ef4444; border-color: #ef4444; background-color: #fef2f2;"
            v-if="['cho_xu_ly', 'dang_chuan_bi'].includes(selectedOrder.status)"
            @click="updateOrderStatus('da_huy')">Huỷ đơn</button>
    ...
    <div class="modal-overlay" v-if="showConfirmModal" @click.self="showConfirmModal = false; pendingStatus = null">
      <div class="modal-box"
        style="max-width: 420px; text-align: center; z-index: 1000; border-radius: 16px; padding: 32px 24px;">
        <div
          style="width: 54px; height: 54px; border-radius: 50%; background: #eff6ff; color: #3b82f6; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px;">
        ...
        <div style="display: flex; gap: 12px; justify-content: center;">
          <button class="btn-ghost" style="padding: 10px 24px; border-radius: 8px;"
            @click="showConfirmModal = false; pendingStatus = null">Huỷ bỏ</button>
          <button class="btn-primary"
            style="background-color: #3b82f6; border-color: #3b82f6; padding: 10px 24px; border-radius: 8px; font-weight: 600; box-shadow: 0 4px 6px -1px rgba(59,130,246,0.5);"
...
export default {
  ...
  data() {
    return {
      search: '', filterPayment: '', activeTab: 'cho_xu_ly',
...
.status-pill.s-cho_xu_ly {
  background-color: #fffbeb !important;
  color: #d97706 !important;
  border: 1px solid #fde68a !important;
}

.status-pill.s-dang_chuan_bi {
  background-color: #eff6ff !important;
  color: #2563eb !important;
  border: 1px solid #bfdbfe !important;
}

.status-pill.s-dang_giao {
  background-color: #fefce8 !important;
  color: #ca8a04 !important;
  border: 1px solid #fef08a !important;
}

.status-pill.s-da_giao {
  background-color: #ecfdf5 !important;
  color: #059669 !important;
  border: 1px solid #a7f3d0 !important;
}

.status-pill.s-da_huy {
