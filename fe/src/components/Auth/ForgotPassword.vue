<template>
  <div class="login-container">
    ...
    <div class="glass-login-card">
      ...
      <form v-if="!isSent" @submit.prevent="handleForgotPassword" class="login-form">
        <!-- Welcoming text -->
        <p class="role-welcome-text">
          Đừng lo lắng! Nhập email của bạn và chúng tôi sẽ gửi liên kết khôi phục mật khẩu.
        </p>
        ...
        <button type="submit" class="submit-btn" :disabled="loading" style="margin-top: 24px;">
          <span v-if="loading"><i class="fa-solid fa-circle-notch fa-spin"></i> Đang gửi...</span>
          <span v-else>
      ...
      <!-- Success State -->
      <div v-else class="login-form" style="text-align: center; padding: 20px 0;">
        <div style="font-size: 48px; color: #10b981; margin-bottom: 16px;">
          <i class="fa-solid fa-envelope-circle-check"></i>
        </div>
        <h2 style="color: #0f172a; font-size: 20px; font-weight: 700; margin-bottom: 12px; font-family: 'DM Sans', sans-serif;">Đã Gửi Thành Công!</h2>
        <p class="role-welcome-text">
          Chúng tôi đã gửi một liên kết khôi phục mật khẩu tới địa chỉ <strong>{{ email }}</strong>. Vui lòng kiểm tra hộp thư đến của bạn.
        </p>
      </div>
      ...
      <div class="card-footer">
        <p class="footer-link-text">
          Nhớ mật khẩu rồi? <router-link to="/login" class="signup-link">Quay lại đăng nhập</router-link>
        </p>
      </div>
    </div>
  </div>
