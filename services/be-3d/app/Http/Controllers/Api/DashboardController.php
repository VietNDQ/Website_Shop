...
class DashboardController extends Controller
{
    public function getDashboardData()
    {
        ...
        if (Schema::hasTable('nhat_ky_hoat_dong')) {
            $logs = NhatKyHoatDong::orderByDesc('tao_luc')->take(10)->get();
            $activities = $logs->map(function($log) {
                $actionText = $log->hanh_dong;
                $ipAddress = null;
                $userAgent = null;

                if (preg_match('/^(.*?)\s*\[IP:\s*([^,]+),\s*UA:\s*(.*?)\]$/s', $log->hanh_dong, $matches)) {
                ...
                return [
                    'id' => $log->id,
                    'user' => $log->ten_nguoi_dung ?? 'Hệ thống',
                    'action' => $actionText,
                    'ip' => $ipAddress,
                    'ua' => $userAgent,
                    'time' => $log->tao_luc ? Carbon::parse($log->tao_luc)->format('H:i - d/m/Y') : 'Vừa xong',
    ...
    public function getAnalyticsData(Request $request)
    {
        ...
        // Tách đơn hủy do khách và nhân viên
        $cancelledOrders = DonHang::with('lichSuTrangThais')
            ->whereIn('trang_thai', ['da_huy', 'hoan_tien'])
            ->whereBetween('tao_luc', [$start, $end])
            ->get();
        ...
        foreach ($cancelledOrders as $order) {
            ...
            $huyLog = $order->lichSuTrangThais->where('trang_thai', 'da_huy')->first() 
                   ?: $order->lichSuTrangThais->where('trang_thai', 'hoan_tien')->first();
            if ($huyLog) {
                $ghiChu = $huyLog->ghi_chu ?? '';
                if (str_contains($ghiChu, 'Khách hàng hủy') || str_contains($ghiChu, 'khach') || str_contains(strtolower($ghiChu), 'khách hàng')) {
                    $nguoiHuy = 'khach';
                }
            } else {
                if ($order->ly_do_huy && (str_contains($order->ly_do_huy, 'Khách') || str_contains($order->ly_do_huy, 'khách'))) {
                    $nguoiHuy = 'khach';
                }
            }
        ...
        $totalStatusOrders = $daGiao + $dangGiao + $choXacNhan + $huyKhach + $huyNhanVien;
        ...
        $legendData = [
            ...
            [
                'label' => 'Hủy do khách hàng',
                'color' => '#ec4899',
                'count' => $huyKhach,
                'pct' => $totalStatusOrders > 0 ? round(($huyKhach / $totalStatusOrders) * 100) : 0
            ],
            [
                'label' => 'Hủy do nhân viên',
                'color' => '#D70018',
                'count' => $huyNhanVien,
                'pct' => $totalStatusOrders > 0 ? round(($huyNhanVien / $totalStatusOrders) * 100) : 0
            ],
        ];
        ...
        foreach ($payments as $payment) {
            $method = strtolower($payment->phuong_thuc);
            if ($method === 'tien_mat' || $method === 'cod') {
                $codAmount += $payment->so_tien;
            } else {
                $ckAmount += $payment->so_tien;
            }
        }

        $totalAmount = $codAmount + $ckAmount;
        $totalPaymentAmount = $totalAmount ?: 1;

        $paymentMethodsList = [
            [
                'name' => 'COD',
                'pct' => $totalAmount > 0 ? round(($codAmount / $totalPaymentAmount) * 100) : 0,
                'amount' => $this->formatCurrency($codAmount),
                'color' => '#f59e0b',
                'bg' => 'rgba(245,158,11,0.1)',
                'icon' => '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>'
            ],
            [
                'name' => 'Chuyển khoản',
                'pct' => $totalAmount > 0 ? round(($ckAmount / $totalPaymentAmount) * 100) : 0,
                'amount' => $this->formatCurrency($ckAmount),
                'color' => '#6366f1',
                'bg' => 'rgba(99,102,241,0.1)',
                'icon' => '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6366f1" stroke-width="2"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 014-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 01-4 4H3"/></svg>'
            ]
        ];
